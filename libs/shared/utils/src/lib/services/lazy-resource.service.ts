...
export class LazyResourceService {
     ...
     */
    loadAll(resources: LazyResource[], targetEl: HTMLElement | string = 'head') {
        return forkJoin(
            resources.map(tuple => {
                const type = tuple[0];
                const url = tuple[1];
...
export declare type LazyResource = ['style'|'script', string, Record<string, string>?];
