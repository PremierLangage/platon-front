...
import { Circle, CircleService } from '@platon/feature/workspace';
...
export class InformationsComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.subscriptions.push(
            this.presenter.contextChange.subscribe(async context => {
                ...
                const circle = this.context.circle as Required<Circle>;
    ...
    async saveChanges() {
        try {
            this.saving = true;
            await this.presenter.update(this.form.value as any);
        } finally {
            this.saving = false;
            this.changeDetectorRef.markForCheck();
        }
