...
import { ResourceFilters, ResourceStatus } from '@platon/feature/workspace';
    ...
    ngOnInit(): void {
        this.subscriptions.push(
            this.form.valueChanges.subscribe(value => {
                this.filter.orderBy = value.order as any;
                this.filter.updatedAt = value.date as any;

                delete this.filter.status;
                if (value.status && value.status !== 'all') {
                    this.filter.status = value.status.toUpperCase() as ResourceStatus;
                }
